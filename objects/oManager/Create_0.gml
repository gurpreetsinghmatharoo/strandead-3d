...
//Background
backTilemap = layer_tilemap_get_id("Tiles_Background");

//Tilemap surface
tSurf = surface_create(room_width, room_height);
tBuff = buffer_create(room_width*room_height*4, buffer_fixed, 1);

surface_set_target(tSurf);
draw_tilemap(backTilemap, 0, 0);
//draw_clear(c_white);
surface_reset_target();

buffer_get_surface(tBuff, tSurf, 0, 0, 0);

layer_set_visible("Tiles_Background", false);
...
//Set invisible
with (all){
	var lN = layer_get_name(layer);
	if (lN=="Instances" || lN=="Shadows" || lN=="Mining"){
		visible = false;
	}
}
...
audio_sound_gain(track, 0, 0); //Mute music
//global.intLevel = 0;
