...
//Get difference
var mHsp, mVsp;
mHsp = global.mouseX - global.mouseXPrev;
mVsp = global.mouseY - global.mouseYPrev;

//Unpaused
if (!global.pause){
	//Apply
	global.camYaw += mHsp;
	global.camPitch -= mVsp;
	global.camPitch = clamp(global.camPitch, -89.9, 89.9);
	
	//Lock
	global.mouseX = wW/2;
	global.mouseY = wH/2;
	window_mouse_set(global.mouseX, global.mouseY);
	cursor_sprite = -1;
}
//Paused
else{
...
//Wrap mouse
//var mwX = window_mouse_get_x();
//var mwY = window_mouse_get_y();
...
if (global.dead){
	if (keyboard_check_pressed(vk_enter)){
		//Reset
		//layer_set_visible("Tiles_Background", true);
		//buffer_delete(tBuff);
		//surface_free(tSurf);
